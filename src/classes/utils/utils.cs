...
namespace UV_Practical3
{
    public class Utils
    {
        ...
        public static SKBitmap CreateBitmapFromIntArray(int[] pixelData, int width, int height)
        {
            if (pixelData.Length != width * height)
                throw new ArgumentException("The length of pixel data does not match the dimensions of the bitmap.");

            // Create a new SKBitmap
            SKBitmap bitmap = new SKBitmap(width, height, SKColorType.Bgra8888, SKAlphaType.Premul);

            // Lock the pixel buffer and copy the data
